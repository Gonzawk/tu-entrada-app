...
import { getMisCajasVentanillaApi } from "../../api/ventanillaApi";
import { AppLayout } from "../../components/shared/AppLayout";
import { RoleGuard } from "../../components/shared/RoleGuard";
import { VentanillaCaja } from "../../types/ventanilla";
import { formatDate } from "../../utils/formatDate";
import { formatMoney } from "../../utils/formatMoney";

const PAGE_SIZE = 10;

export default function VentanillaCajasScreen() {
  const [items, setItems] = useState<VentanillaCaja[]>([]);
  ...
  async function loadFirstPage(isRefreshing = false) {
    try {
      ...
      const result = await getMisCajasVentanillaApi({
        page: 1,
        pageSize: PAGE_SIZE,
      });
  ...
  function renderItem({ item }: { item: VentanillaCaja }) {
    const montoInicial = item.montoInicial ?? 0;
    const ventasEfectivo = item.totalEfectivo ?? 0;
    const ventasTransferencia = item.totalTransferencia ?? 0;
    const totalEfectivoARendir =
      item.totalEfectivoARendir ?? montoInicial + ventasEfectivo;
    const totalGeneralARendir =
      item.totalGeneralARendir ?? montoInicial + (item.totalGeneral ?? 0);
    ...
    return (
      <Pressable
        ...
        onPress={() =>
          router.push({
            pathname: "/ventanilla/caja-detail",
      ...
      >
        ...
        <View style={styles.summaryBox}>
          ...
          <Row label="Transferencia" value={formatMoney(ventasTransferencia)} />
