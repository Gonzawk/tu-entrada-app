...
import { getMiCajaBarraDetalleApi } from "../../api/barraApi";
import { AppLayout } from "../../components/shared/AppLayout";
import { RoleGuard } from "../../components/shared/RoleGuard";
import { BarraCajaDetalle } from "../../types/barra";
...
export default function BarraCajaDetailScreen() {
  const { cajaId } = useLocalSearchParams<{ cajaId: string }>();

  const [loading, setLoading] = useState(true);
  const [detalle, setDetalle] = useState<BarraCajaDetalle | null>(null);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const data = await getMiCajaBarraDetalleApi(id);
      setDetalle(data);
    } catch (e: unknown) {
      Alert.alert(
        "Error",
  ...
  const totalEfectivoARendir = detalle.montoInicial + ventasEfectivo;
  const totalGeneralARendir = detalle.montoInicial + detalle.totalGeneral;
    ...
    <RoleGuard allowedRoles={["Barra"]}>
      <AppLayout title="Detalle caja">
        ...
        <View style={styles.card}>
          ...
          <MoneyRow label="Transferencias" value={detalle.totalTransferencia} />
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.muted}>
            Ventas registradas: {detalle.cantidadVentas}
          </Text>
