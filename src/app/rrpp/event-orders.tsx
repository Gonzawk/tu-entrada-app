...
import { getHistorialEventoRRPPApi } from "../../api/rrppApi";
...
const ESTADOS = [
  ...
  { label: "Pendientes", value: "PendientePago" },
...
export default function RRPPEventOrdersScreen() {
  ...
  const loadFirstPage = useCallback(
    async (isRefreshing = false) => {
      try {
        ...
        const result = await getHistorialEventoRRPPApi({
          eventoId: id,
          page: 1,
          pageSize: PAGE_SIZE,
    ...
    [id, debouncedSearch, estado]
  );

  useEffect(() => {
    loadFirstPage(false);
  }, [id, debouncedSearch, estado, loadFirstPage]);
  ...
  function renderItem({ item }: { item: RRPPOrdenEvento }) {
    return (
      <View style={styles.card}>
        ...
        <Text style={getEstadoStyle(item.estado)}>Estado: {item.estado}</Text>
  ...
  return (
    <RoleGuard allowedRoles={["RRPP"]}>
      <AppLayout title="Órdenes evento" scroll={false}>
        <View style={styles.screen}>
          <View style={styles.resumeCard}>
            <Text style={styles.resumeLabel}>Total cargado</Text>
            <Text style={styles.resumeAmount}>{formatMoney(totalCargado)}</Text>
            <Text style={styles.resumeText}>
              {items.length} órdenes cargadas · {ticketsCargados} tickets
            </Text>
          </View>
          ...
          <View style={styles.filters}>
            {ESTADOS.map((x) => (
              <Pressable
                key={x.label}
                style={estado === x.value ? styles.filterActive : styles.filter}
                onPress={() => setEstado(x.value)}
              >
                <Text style={styles.filterText}>{x.label}</Text>
              </Pressable>
            ))}
          </View>
...
const styles = StyleSheet.create({
  ...
  resumeLabel: { color: "#BDBDBD", fontWeight: "900" },
  resumeAmount: {
    color: "#FFFFFF",
    fontSize: 32,
    fontWeight: "900",
  ...
  resumeText: { color: "#BDBDBD", marginTop: 5 },
  searchInput: {
    minHeight: 48,
    borderRadius: 16,
  ...
  filterText: { color: "#FFFFFF", fontWeight: "900" },
  card: {
  ...
  pending: { color: "#FFD166", marginTop: 8, fontWeight: "900" },
  cancelled: { color: "#FF4D57", marginTop: 8, fontWeight: "900" },
  emptyCard: {
    backgroundColor: "rgba(255,255,255,0.07)",
    borderRadius: 22,
    padding: 16,
